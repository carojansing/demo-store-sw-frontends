...
<template>
  ...
  <div class="sw-product-card group relative flex flex-col justify-between" data-testid="product-box">
    <div :class="[
      'w-full rounded-md overflow-hidden hover:opacity-75',
      layoutType === 'image' ? 'h-80' : 'h-60',
    ]">
      <img :src="getProductThumbnailUrl(product)" :alt="getProductName({ product }) || ''" :class="{
          'w-full h-full': true,
          'object-cover':
            displayMode === 'cover' ||
    ...
    <button aria-label="Add to wishlist" type="button" class="absolute top-2 right-2"
      data-testid="product-box-toggle-wishlist-button" @click="addToWishlistFn">
      <div class="h-7 w-7" :class="{
    ...
    <div class="mt-4 flex flex-col justify-between flex-1">
      <div>
        <h3 class="text-base font-bold text-gray-700">
          <NuxtLink class="line-clamp-2 h-12" :to="getProductUrl(product)" data-testid="product-box-product-name-link">
            {{ getProductName({ product }) }}
          </NuxtLink>
        </h3>
        <div v-if="layoutType === 'standard'" class="line-clamp-4 mt-2 text-sm text-gray-500 h-20 overflow-hidden"
          data-testid="product-box-product-description" v-html="getTranslatedProperty(product, 'description')" />
        <div class="mt-2 flex gap-2 flex-wrap">
          <span v-for="option in product?.options" :key="(option as PropertyGroupOption).id"
            class="bg-gray-400 text-sm text-white rounded py-1 px-2" data-testid="product-box-product-options">
      ...
      <div class="flex flex-col mt-3 justify-between">
        <SharedListingProductPrice :product="product" class="ml-auto" data-testid="product-box-product-price" />
        <div class="sw-product-rating inline-flex" data-testid="product-box-product-rating">
    ...
    <div class="mt-3">
      <button v-if="!fromPrice" type="button"
